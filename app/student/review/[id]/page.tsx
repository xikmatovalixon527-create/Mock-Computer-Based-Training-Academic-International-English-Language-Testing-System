...
export default function StudentReviewResult({ params }: StudentReviewResultProps) {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        const [essayRes, reviewRes] = await Promise.all([
          fetch(`/api/essays?id=${id}`),
          fetch(`/api/reviews?essay_id=${id}`)
        ]);
        ...
        if (essayData.essay) {
          ...
          if (essayData.essay.task_type === 'task2') {
            setActiveTaskTab(1);
          }
        }
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#0a0a0c] flex flex-col items-center justify-center gap-3">
        ...
        <p className="text-xs text-[#8a8a8e] uppercase tracking-wider font-mono">Syncing review results...</p>
      </div>
    );
  }
  ...
  if (!essay) {
    return (
      <div className="min-h-screen bg-[#0a0a0c] p-8 flex flex-col items-center justify-center space-y-4">
        <p className="text-[#ff453a] text-sm font-semibold uppercase tracking-wider">Submission Not Found</p>
        <button 
          onClick={() => router.push('/student/dashboard')}
          className="px-4 py-2 bg-zinc-900 border border-zinc-800 text-xs font-bold uppercase rounded-full hover:bg-zinc-800 transition-colors"
        >
          Return to Dashboard
        </button>
      </div>
  ...
  try {
    topicData = JSON.parse(essay.topic_text);
  } catch {
    topicData = {
      task1: { text: essay.topic_text, images: [] },
      task2: { text: essay.topic_text, images: [] }
    };
  }
  ...
  let activeScores = {
    ta_band: review?.ta_band,
    ta_feedback: review?.ta_feedback,
    cc_band: review?.cc_band,
    cc_feedback: review?.cc_feedback,
    lr_band: review?.lr_band,
    lr_feedback: review?.lr_feedback,
    gra_band: review?.gra_band,
    gra_feedback: review?.gra_feedback
  };
  ...
  try {
    if (review?.overall_feedback?.startsWith('{')) {
      ...
      if (parsed.task1 && parsed.task2) {
        ...
        activeScores = {
          ta_band: activeStats.scores.ta,
          ta_feedback: activeStats.feedbacks.ta,
          cc_band: activeStats.scores.cc,
          cc_feedback: activeStats.feedbacks.cc,
          lr_band: activeStats.scores.lr,
          lr_feedback: activeStats.feedbacks.lr,
          gra_band: activeStats.scores.gra,
          gra_feedback: activeStats.feedbacks.gra
        };
  ...
  const isReviewed = essay.status === 'reviewed';
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 bg-[#0a0a0c] min-h-screen text-[#f5f5f7] p-4 sm:p-6 lg:p-8 relative">
      ...
      <div className="flex items-center space-x-2 text-[10px] uppercase tracking-wider text-[#8a8a8e] mb-4">
        ...
        <span className="font-semibold text-white">Evaluation Results</span>
      </div>
          ...
          {showTabs && (
            <div className="flex border-b border-[#1f1f23] bg-[#121214] p-1 rounded-t-lg">
              <button 
                ...
                className={`flex-1 py-2 text-xs uppercase tracking-wider font-semibold cursor-pointer transition-all ${
                  activeTaskTab === 0 ? 'bg-black text-white border-b border-white' : 'text-[#8a8a8e] hover:text-white'
                }`}
              >
                Task 1
              </button>
              <button 
                ...
                className={`flex-1 py-2 text-xs uppercase tracking-wider font-semibold cursor-pointer transition-all ${
                  activeTaskTab === 1 ? 'bg-black text-white border-b border-white' : 'text-[#8a8a8e] hover:text-white'
                }`}
              >
                Task 2
              </button>
          ...
          <div className="bg-[#121214] p-5 rounded-lg border border-[#1f1f23]">
            ...
            {activeTopic?.images && activeTopic.images.length > 0 ? (
              <div className="space-y-2 mb-4">
                {activeTopic.images.map((img: string, idx: number) => (
                  <img 
                    key={idx} 
                    src={img} 
                    alt={`Task diagram ${idx + 1}`} 
                    onClick={() => setLightboxImg(img)} 
                    className="max-h-[300px] w-full object-contain rounded cursor-zoom-in border border-[#1f1f23] transition-transform duration-200 hover:scale-[1.01]" 
                  />
                ))}
              </div>
            ) : activeTopic?.image ? (
              <img 
                ...
                alt="Task diagram" 
                onClick={() => setLightboxImg(activeTopic.image)} 
                className="max-h-[300px] w-full object-contain rounded mb-4 cursor-zoom-in border border-[#1f1f23] transition-transform duration-200 hover:scale-[1.01]" 
              />
            ) : null}
            ...
            <div className="p-6 bg-black/60 min-h-[300px]">
              {isReviewed ? (
                <HighlightedText 
                  ...
                  taskIndex={activeTaskTab} 
                  focusedCommentIndex={focusedCommentIndex} 
                  setFocusedCommentIndex={setFocusedCommentIndex} 
                />
              ) : (
            ...
            <>
              ...
              <div className="bg-[#121214] rounded-lg border border-[#1f1f23] overflow-hidden">
                <div className="divide-y divide-[#1f1f23]">
                  {[
                    { name: 'Task Achievement (TA)', score: activeScores.ta_band, fb: activeScores.ta_feedback },
                    { name: 'Coherence & Cohesion (CC)', score: activeScores.cc_band, fb: activeScores.cc_feedback },
                    { name: 'Lexical Resource (LR)', score: activeScores.lr_band, fb: activeScores.lr_feedback },
                    { name: 'Grammatical Range (GRA)', score: activeScores.gra_band, fb: activeScores.gra_feedback }
                  ].map((crit, idx) => (
                    <div key={idx} className="p-4 space-y-2">
                      <div className="flex justify-between items-center">
                        <div className="font-semibold text-xs uppercase tracking-wider text-[#8a8a8e]">{crit.name}</div>
                        <div className={`text-xs px-2.5 py-1 rounded border font-mono ${getBandBadgeStyle(crit.score)}`}>
                          {crit.score ? Number(crit.score).toFixed(1) : '-'}
                        </div>
                      </div>
